...
$('cinematicReelBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (serverPaths.length === 0 && state.photos.length > 0) {
            setProgress(6, `📤 Uploading ${state.photos.length} photo(s) to server… (this may take a moment)`);
            try {
                const fd = new FormData();
                state.photos.forEach(f => fd.append('files', f));
                const up = await fetch(`${API}/images/upload`, {
                    method: 'POST', body: fd,
                    signal: AbortSignal.timeout(300_000), // 5 minutes for large batches
                });
                if (up.ok) {
                    ...
                    setProgress(8, `✅ ${ranked.length} photos uploaded & ranked — starting cinematic pipeline…`);
                } else {
                    const errBody = await up.text().catch(() => `HTTP ${up.status}`);
                    console.warn('[Cinematic] Upload returned', up.status, errBody);
                    showErr('storyErr', `⚠ Upload failed (${up.status}). Try using the Analyse button first, then Generate Cinematic Reel.`);
                }
            } catch (uploadErr) {
                console.warn('[Cinematic] Upload failed:', uploadErr.message);
                showErr('storyErr', `⚠ Upload error: ${uploadErr.message}. Make sure the backend is running at http://localhost:8000 and try the Analyse button first.`);
            }
        }
        ...
        if (serverPaths.length === 0) {
            ...
            let errMsg = '⚠ ';
            if (!hasLocalPhotos) {
                errMsg += 'No photos selected. Please drag & drop photos in Step 1, click "Analyse & Generate Reel Story", then try again.';
            } else {
                errMsg += 'Photos could not be uploaded to the server. Please click the "✨ Analyse & Generate Reel Story" button first, wait for it to complete, then click Generate Cinematic Reel.';
            }
            showErr('storyErr', errMsg);
            console.error('[Cinematic] serverPaths empty. state.photos:', state.photos.length);
            return;
        }
