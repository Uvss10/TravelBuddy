...
def _get_cache_path(image_path: str) -> Path:
    ...
    base_hash = hashlib.sha256(f"{image_path}_{mtime}".encode()).hexdigest()[:16]
...
def _analyze_single(path: str) -> Optional[PhotoMeta]:
    ...
    # Implement quick caching to skip analysis of unmodified files
    ...
    try:
        ...
        print(f"[Vision] Ignoring cache write error: {e}")
