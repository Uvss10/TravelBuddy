...
# ─────────────────────────────────────────────────────────────────────────────
# PUBLIC: call_llm  —  ALWAYS uses Groq Cloud API.
#   Falls back to smart mock ONLY if Groq is completely unreachable.
#   Ollama / local mode is NEVER used automatically.
# ─────────────────────────────────────────────────────────────────────────────
def call_llm(prompt: str, model: str = None) -> str:
    """Always uses Groq Cloud API. Smart mock is the only fallback (no Ollama)."""
    print("[LLM] Using Groq Cloud API (forced — Ollama disabled)")
    try:
        return _call_groq(prompt)
    except Exception as e:
        print(f"[LLM] Groq failed: {e} — falling back to offline knowledge base")
    return _smart_mock(prompt)


def call_vision_llm(image_path: str, prompt: str) -> str:
    """Always uses Groq Vision API. Returns error JSON if Groq is unreachable."""
