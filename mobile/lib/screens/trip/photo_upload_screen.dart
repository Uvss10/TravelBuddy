...
class _PhotoUploadScreenState extends State<PhotoUploadScreen> {
  ...
  Future<void> _pickAudio() async {
    ...
    final choice = await showModalBottomSheet<String>(
      ...
      builder: (context) => Column(
        ...
        children: [
          ...
          const Text('Select Soundtrack', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          const SizedBox(height: 16),
          ListTile(
            leading: const Icon(Icons.library_music_rounded, color: AppTheme.primary),
            title: const Text('Choose from Library'),
            subtitle: const Text('Copyright-free cinematic scores'),
            onTap: () => Navigator.pop(context, 'library'),
          ),
          ListTile(
            leading: const Icon(Icons.file_upload_rounded, color: AppTheme.primary),
            title: const Text('Upload Custom File'),
            subtitle: const Text('Select mp3/wav from your phone'),
            onTap: () => Navigator.pop(context, 'upload'),
          ),
          if (_selectedAudio != null || _serverAudioPath != null)
            ListTile(
              leading: const Icon(Icons.remove_circle_outline_rounded, color: AppTheme.error),
              title: const Text('Remove Soundtrack'),
    ...
    if (choice == 'library') {
      final track = await showDialog<Map<String, dynamic>>(
        context: context,
      ...
      if (track != null) {
        setState(() {
          ...
          _serverAudioPath = track['path'];
          _selectedAudio = null; // Clear local file if any
        });
      }
  ...
  Future<void> _proceed() async {
    ...
    context.read<VideoGenerationProvider>().startCinematicGeneration(
      ...
      audioPath: _isLibraryTrack ? _serverAudioPath : _selectedAudio?.path,
    );
